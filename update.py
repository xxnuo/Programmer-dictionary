...
url_md_all = 'https://raw.githubusercontent.com/GrowingGit/GitHub-English-Top-Charts/main/content/charts/overall/software/All-Language.md'
path_md_all = os.path.join(path_data, 'All-Language.md')

if IFUPDATE or not os.path.exists(path_md_all):
    try:
        r = requests.get(url_md_all, headers=headers, timeout=10)
        my.log('url_md_all:' + str(r.status_code))
        if r.status_code == 200:
            # 写入md文件
            with open(path_md_all, 'w', encoding='utf-8') as f:
                f.write(r.text)
    ...
    my.log('3.已获取到All-Language.md，开始解析')
